...
{
public:
	const linalg::vec4f forward = { 0,0,-1, 0 };
	const linalg::vec4f right = { 1,0,0, 0 };
	const linalg::vec4f up = { 0,1,0, 0 };
	/**
	...
	*/
	...
	void Rotate(const linalg::vec3f& rotation) noexcept;

	linalg::mat4f RotationMatrix(const float& roll, const float& yaw, const float& pitch) const noexcept;
	linalg::vec3f GetPosition() const noexcept;
	linalg::vec4f GetWorldSpacePosition() const noexcept;
private:
	// Aperture attributes
